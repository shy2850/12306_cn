...
public class UserDao {
	...
	public int save(User user) throws SQLException{
		return dbmanager.executeUpdate("insert into user values(?,?,?,?,?,?,?,?,?)", show_sql, user.getId(),user.getName(),user.getPassword(),user.getEmail(),user.getId_card(),user.getNative_place(),user.getBirth(),user.getSex(),user.getType());
	}//save()
	...
	public int update(User user) throws SQLException{
		return dbmanager.executeUpdate("update user set name=?,password=?,email=?,id_card=?,native_place=?,birth=?,sex=?,type=? where id=?", show_sql, user.getName(),user.getPassword(),user.getEmail(),user.getId_card(),user.getNative_place(),user.getBirth(),user.getSex(),user.getType(),user.getId());
	}//update()
